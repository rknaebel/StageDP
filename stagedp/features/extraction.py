...
class ActionFeatureGenerator:
    ...
    def gen_features(self):
        ...
        """
        feat_list = list(chain(
            self.status_features(),
            self.operational_features(),
            self.organizational_features(),
            self.syntactic_featues(),
            self.structural_features(),
            self.ngram_features(),
            self.nucleus_features()))
        if self.bcvocab is not None:
            feat_list += list(self.bc_features())
        return dict((fs, 1) if isinstance(fs, str) else fs for fs in feat_list)
    ...
    def operational_features(self):
        ...
        if len(self.action_hist) == 0:
            yield 'Last-Action', 'None'
            yield 'Second-Last-Action', 'None'
        elif len(self.action_hist) == 1:
            yield 'Last-Action', action2str(self.action_hist[-1])
            yield 'Second-Last-Action', 'None'
        else:
            yield 'Last-Action', action2str(self.action_hist[-1])
            yield 'Second-Last-Action', action2str(self.action_hist[-2])
    ...
    def structural_features(self):
        ...
        if self.top1span is not None and self.top2span is not None \
            ...
            yield 'Top12-Stack-Form', f"{self.top1span.form},{self.top2span.form}"
        # distance
        if self.top1span is not None:
        ...
        if (self.top1span is not None) and (self.top2span is not None):
            if edulen1 > edulen2:
                yield 'Top-Stack-EDU-Comparison', 'Less'
            elif edulen1 < edulen2:
                yield 'Top-Stack-EDU-Comparison', 'Greater'
            else:
        ...
        if (self.top1span is not None) and (self.top2span is not None):
            if sentlen1 > sentlen2:
                yield 'Top-Stack-Sent-Comparison', 'Greater'
            elif sentlen1 < sentlen2:
                yield 'Top-Stack-Sent-Comparison', 'Less'
            else:
        ...
        for span_name, span in [('Top1', self.top1span), ('Top2', self.top2span), ('Queue', self.firstspan)]:
            ...
            for idx, head in enumerate(text_heads):
                if head not in text_tidx:
                    ...
                    yield f'{span_name}-Nuc-EDU-head-word', head_token.lemma
                    yield f'{span_name}-Nuc-EDU-head-pos', head_token.pos
                    yield f'{span_name}-Nuc-EDU-head-dep', text_deps[idx]
                    # if self.top1span is not None and self.top2span is not None:
...
class RelationFeatureGenerator:
    def __init__(self, node, rst_tree, level, bcvocab, nprefix=11):
        ...
        self.doc = rst_tree.doc
        self.root = rst_tree.tree
        self.bcvocab = bcvocab
        self.nprefix = nprefix
        # Doc length wrt EDUs
        self.doclen = len(self.doc.edu_dict)
    ...
    def gen_features(self):
        ...
        if self.level in [1, 2]:
            feat_list += list(self.form_features())
        if self.level in [1, 2]:
            feat_list += list(self.tree_features())
        if self.level in [1, 2]:
            feat_list += list(self.nucleus_features())
        if self.level in [0]:
            feat_list += list(self.syntactic_features())
    ...
    def lexical_features(self):
        ...
        for gram in get_grams(left_text, self.doc.token_dict):
            yield 'Lnode-nGram', gram
        for gram in get_grams(right_text, self.doc.token_dict):
            yield 'Rnode-nGram', gram
        for gram in get_conjunctive_grams(left_text, right_text, self.doc.token_dict):
    ...
    def structural_features(self):
        ...
        if self.level == 0:
            ...
            if (self.lnode is not None) and (self.rnode is not None):
                ...
                if edulen1 > edulen2:
                    yield 'LRnode-EDU-Comparison', 'Greater'
                elif edulen1 < edulen2:
                    yield 'LRnode-EDU-Comparison', 'Less'
                else:
        ...
        if self.level == 1:
            ...
            if (self.lnode is not None) and (self.rnode is not None):
                ...
                if sentlen1 > sentlen2:
                    yield 'LRnode-Sent-Comparison', 'Greater'
                elif sentlen1 < sentlen2:
                    yield 'LRnode-Sent-Comparison', 'Less'
                else:
        ...
        if self.level == 2:
            ...
            if (self.lnode is not None) and (self.rnode is not None):
                ...
                if paralen1 > paralen2:
                    yield 'LRnode-Para-Comparison', 'Greater'
                elif paralen1 < paralen2:
                    yield 'LRnode-Para-Comparison', 'Less'
                else:
...
def get_grams(text, token_dict):
    ...
    grams = set()
    if n >= 1:
        grams.add(f'Start-Unigram-Word-{token_dict[text[0]].lemma.lower()}')
        grams.add(f'End-Unigram-Word-{token_dict[text[-1]].lemma.lower()}')
        grams.add(f'Start-Unigram-Pos-{token_dict[text[0]].pos.lower()}')
        grams.add(f'End-Unigram-Pos-{token_dict[text[-1]].pos.lower()}')
    if n >= 2:
        grams.add(f'Start-Bigram-Word-{token_dict[text[0]].lemma.lower()}+{token_dict[text[1]].lemma.lower()}')
        grams.add(f'Start-Bigram-Pos-{token_dict[text[0]].pos.lower()}+{token_dict[text[1]].pos.lower()}')
        grams.add(f'End-Bigram-Word-{token_dict[text[-2]].lemma.lower()}+{token_dict[text[-1]].lemma.lower()}')
        grams.add(f'End-Bigram-Pos-{token_dict[text[-2]].pos.lower()}+{token_dict[text[-1]].pos.lower()}')
    return grams
